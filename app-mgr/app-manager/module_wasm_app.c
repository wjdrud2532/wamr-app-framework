...
#if WASM_ENABLE_INTERP != 0 || WASM_ENABLE_JIT != 0
static void destroy_all_wasm_sections(wasm_section_list_t sections);

static void destroy_part_wasm_sections(wasm_section_list_t *p_sections,
                                       uint8 *section_types, int section_cnt);
#endif

#if WASM_ENABLE_AOT != 0
static void destroy_all_aot_sections(aot_section_list_t sections);

static void destroy_part_aot_sections(aot_section_list_t *p_sections,
                                      uint8 *section_types, int section_cnt);
#endif
...
static void exchange_uint32(uint8 *p_data) {
  uint8 value = *p_data;
  *p_data = *(p_data + 3);
  *(p_data + 3) = value;
...
static void app_instance_queue_callback(void *queue_msg, void *arg) {
  ...
  if (message_type < BASE_EVENT_MAX) {
    ...
    case RESTFUL_REQUEST: {
      ...
      buffer_offset = wasm_runtime_module_dup_data(inst, buffer, (uint64)size);
      ...
      if (!wasm_runtime_call_wasm(wasm_app_data->exec_env, func_onRequest, 2,
                                  argv)) {
        const char *exception = wasm_runtime_get_exception(inst);
        bh_assert(exception);
        app_manager_printf("Got exception running wasm code: %s\n", exception);
    ...
    case RESTFUL_RESPONSE: {
      ...
      func_onResponse = app_manager_lookup_function(inst, "_on_response");
      if (!func_onResponse) {
      ...
      if (!wasm_runtime_call_wasm(wasm_app_data->exec_env, func_onResponse, 2,
                                  argv)) {
        const char *exception = wasm_runtime_get_exception(inst);
        bh_assert(exception);
        app_manager_printf("Got exception running wasm code: %s\n", exception);
    ...
    case TIMER_EVENT_WASM: {
      ...
      if (bh_message_payload(queue_msg)) {
        ...
        func_onTimer = app_manager_lookup_function(inst, "_on_timer_callback");

        if (!func_onTimer) {
          app_manager_printf("Cannot find function _on_timer_callback\n");
          break;
        }
        timer_id = (unsigned int)(uintptr_t)bh_message_payload(queue_msg);
        argv[0] = timer_id;
        if (!wasm_runtime_call_wasm(wasm_app_data->exec_env, func_onTimer, 1,
                                    argv)) {
          const char *exception = wasm_runtime_get_exception(inst);
          bh_assert(exception);
          app_manager_printf("Got exception running wasm code: %s\n",
...
static bool wasm_app_prepare_wasi_dir(wasm_module_t module,
...
static void *wasm_app_routine(void *arg) {
  ...
  if (!wasm_runtime_call_wasm(wasm_app_data->exec_env, func_onInit, 0, NULL)) {
    const char *exception = wasm_runtime_get_exception(inst);
    bh_assert(exception);
    app_manager_printf("Got exception running WASM code: %s\n", exception);
    wasm_runtime_clear_exception(inst);
...
static bool wasm_app_module_install(request_t *msg) {
  ...
  case Wasm_Module_AoT: {
    ...
    module = wasm_runtime_load_from_sections(aot_file->sections, true, err,
                                             err_size);
    if (!module) {
    ...
    wasm_runtime_set_wasi_args(module, wasi_dir_list, 1, NULL, 0, NULL, 0, NULL,
                               0);
#endif
  ...
  if (!watchdog_timer_init(m_data)) {
    SEND_ERR_RESPONSE(
        msg->mid, "Install WASM app failed: create app watchdog timer failed.");
    goto fail;
  }
...
wasm_app_module_on_install_request_byte_arrive(uint8 ch, int request_total_size,
  ...
  *received_size = recv_ctx.total_received_size;
  ...
  if (recv_ctx.phase == Phase_Req_Ver) {
    ...
    p[recv_ctx.size_in_phase++] = ch;
    if (recv_ctx.size_in_phase == sizeof(recv_ctx.message.request_url_len)) {
...
#if WASM_ENABLE_AOT != 0
      case Wasm_Module_AoT:
        recv_ctx.message.app_file.u.aot.magic = recv_ctx.message.app_file_magic;
    ...
    if (section_type <= section_type_max) {
      ...
      if (!(new_section =
                (wasm_section_t *)APP_MGR_MALLOC(sizeof(wasm_section_t)))) {
        app_manager_printf("Allocate memory failed!\n");
        SEND_ERR_RESPONSE(recv_ctx.message.request_mid,
                          "Install WASM app failed: "
                          "allocate memory failed.");
    ...
    section->section_body_size |= ((byte & 0x7f) << recv_ctx.size_in_phase * 7);
...
#endif
          uint64 total_size =
              (uint64)section->section_body_size + aot_get_plt_table_size();
    ...
    if (recv_ctx.size_in_phase == section->section_body_size) {
      if (section->section_type == AOT_SECTION_TYPE_TEXT) {
        ...
        if (total_size > section->section_body_size) {
          memset(section->section_body + section->section_body_size, 0,
                 total_size - section->section_body_size);
...
bool wasm_set_wasi_root_dir(const char *root_dir) {
...
const char *wasm_get_wasi_root_dir() { return wasi_root_dir; }
#endif
